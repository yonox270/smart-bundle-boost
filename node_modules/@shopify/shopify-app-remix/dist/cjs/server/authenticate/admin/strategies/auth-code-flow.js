...
class AuthCodeFlowStrategy {
    ...
    async authenticate(request, sessionContext) {
        ...
        if (!session) {
            ...
            await redirectToAuthPage.redirectToAuthPage({ config, api }, request, shop);
        }
        ...
        return handleClientError.handleClientErrorFactory({
            ...
            onError: async ({ session, error }) => {
                if (error.response.code === 401) {
                    throw await redirectToAuthPage.redirectToAuthPage({ api, config}, request, session.shop);
                }
            },
        });
    }
    async ensureInstalledOnShop(request) {
        const { api, config, logger } = this;
        validateShopAndHostParams.validateShopAndHostParams({ api, config, logger }, request);
        const url = new URL(request.url);
        ...
        if (!offlineSession) {
            ...
            if (isEmbedded) {
                redirectWithExitiframe.redirectWithExitIframe({ api, config}, request, shop);
            }
            else {
                throw await beginAuth.beginAuth({ api, config}, request, false, shop);
            }
        }
    ...
    async handleAuthBeginRequest(request, shop) {
        ...
        if (config.isEmbeddedApp &&
            ...
            throw redirectWithExitiframe.redirectWithExitIframe({ api, config}, request, shop);
        }
        else {
            throw await beginAuth.beginAuth({ api, config}, request, false, shop);
        }
    ...
    async handleAuthCallbackRequest(request, shop) {
        ...
        try {
            ...
            if (config.useOnlineTokens && !session.isOnline) {
                ...
                await beginAuth.beginAuth({ api, config, logger }, request, true, shop);
            }
            logger.debug('Request is valid, loaded session from OAuth callback', {
                shop: session.shop,
                isOnline: session.isOnline,
            });
            await triggerAfterAuthHook.triggerAfterAuthHook({ api, config, logger }, session, request, this);
            throw await redirectToShopifyOrAppRoot.redirectToShopifyOrAppRoot(request, { api, config, logger }, responseHeaders);
        }
        catch (error) {
    ...
    async oauthCallbackError(error, request, shop) {
        ...
        if (error instanceof shopifyApi.InvalidHmacError ||
            error instanceof shopifyApi.InvalidOAuthError) {
